...
#' @param file.range range of file numbers to be processed (i.e c(0,10))
...
#'       
#'       
CalcFlow=function(file.path, run.name, file.range, slopex.file,
                  slopey.file, overland.type, mannings, epsilon=1e-5,
                  dx=1, dy=1, mask){
  
  
#Get the range of file number to be read in   
file.nums=file.range[1]:file.range[2]
nfile=length(filenums)

#make a zeros matrix for pmax calcs later
zeros=matrix(0, nrow=nx, ncol=ny)

#read in the slope files
slopex=sprintf("%s/%s", file.path, slopex.file)
slopey=sprintf("%s/%s", file.path, slopyx.file)
nx=dim(slopex)[1]
ny=dim(slopex)[2]
...
for(f in 1:nfile){
  ...
  #Write the outputs
  outflow.file=sprintf("%s/%s.out.outflow.%05d.pfb", file.path, run.name, file.number)
  writepfb(outflow, outflow.file, dx, dy, 1)

  #Write the outputs
  qeast.file=sprintf("%s/%s.out.q_east.%05d.pfb", file.path, run.name, file.number)
  writepfb(qeast, qeast.file, dx, dy, 1)

  #Write the outputs
  qnorth.file=sprintf("%s/%s.out.q_east.%05d.pfb", file.path, run.name, file.number)
  writepfb(qnorth, qeast.north, dx, dy, 1)
