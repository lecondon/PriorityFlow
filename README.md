...
+ Border cells: By default all border cells are processed to point out. An optional border direction mask can be input to set some borders to point in (Note that if you use this option the default will be to have border cells point out, however wherever there is one or more cells draining to a border cell this will pointed out to allow surface water bodies to exit).
